...
def main():
	...
	for i in range(len(runstrs)):
		...
		#scaleout = np.ones((data.shape[0]//2,data.shape[1]))*info_int8_t.max
		'''
		Be careful here.  Generally we are treating the "rows" in the original images as the "features" 
		while each image is treated as a "sample" and therefore a "row" in the processed objects.
		This is likely to get confusing, so try to use headers in file writes to make things clear.
		'''
		...
		for row in range(0,2**10):
			...
			header = 'histogram'
			np.savetxt(filename,h,fmt='%i',header = header)
