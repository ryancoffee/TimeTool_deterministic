...
#runstrs = ['29'] #['136','131','137','138'] # ['93']#,'94','97']#'21','32']#'15']#'136','137','138']#'119','74','77','76','75','84'];#['15'];#,'13','14','15','9','10','9','10']; # 
#runstrs = ['95']
runstrs = ['134','134','136','136']
...
vwins = [(575,585)]*len(runstrs) # [(620,630)]; # [(210,220)] # [(480,500)]*len(runstrs)#(575,585),(570,580),(580,590)]#,(480,500),(480,500),(480,500),(480,500)]; #,(440,450),(440,450),(577,587),(577,587),(577,587),(577,587)]; # this is the integration window for the stripe projection
...
subrefs = [True,False,True,False] #[False]*len(runstrs)#,True,True,True,True,True,False,False);
...
#expstrs = [str('xppl3816')]*len(runstrs)
...
dets = ['opal_0']*len(runstrs) 
dets = ['opal_1']*len(runstrs) 
...
for i in range(len(runstrs)):
	...
	det = Detector(dets[i]) # for AMO
	GDdet = Detector('FEEGasDetEnergy'); #This I hope is the FEE Gas Detector readings
	EBdet = Detector('EBeam'); #This I hope is the BLD data
	evr = Detector('NoDetector.0:Evr.0');
	cd = Detector('ControlData');
	runipms = bool(False)
	if regexp.search('^.*(xpp).*',expstr):
		runipms = True
	if runipms:
		ipm1 = Detector('HX2-SB1-BMMON');
		ipm2 = Detector('XPP-SB2-BMMON');
		ipm3 = Detector('XPP-SB3-BMMON');
